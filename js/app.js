...
// ---------- ИЗОБРАЖЕНИЯ ДЛЯ ТИПАЖЕЙ ----------
const TYPE_IMAGES = {
    'Стратег': 'img/types/strateg.png',
    'Городской предприниматель': 'img/types/gorodskoy.png',
    'Консервативный скряга': 'img/types/skryaga.png',
    'Рискованный стартапер': 'img/types/startaper.png',
    'Бизнесмен-визионер': 'img/types/vizioner.png'
};

// ---------- ФУНКЦИЯ РАСЧЁТА СТОИМОСТИ ЛОТА ----------
...
// ---------- ПОДСКАЗКИ ----------
const HINTS = {
    1: "Следующий объект охраняет самый настоящий ветеран железных дорог. Он не двигался уже много лет, но именно он привёз сюда всех остальных",
    2: "Вокзал всегда был романтичным местом для двоих",
    3: "Четвёртый объект не прячется — наоборот, он там, где людей больше всего. Идите на шум и толкучку, не ошибётесь",
    4: "Последний объект — рядом с той, кого здесь знает каждый. Про Любу Кулакову, слышали?"
};
...
function calculateResult() {
    ...
    if (takenCount <= 1) {
        return {
            type: 'Стратег',
            desc: 'Семь раз проверит — один раз подпишет. Не поддаётся азарту квеста и предпочитает уйти ни с чем.'
        };
    }
    ...
    if (takenCount > 1 && hasCapital) {
        return {
            ...
            desc: 'Локация решает всё. Готов переплатить за адрес, который сам себя продаёт.'
        };
    }
    ...
    if (takenCount > 1 && !hasCapital && riskyCount === 0) {
        return {
            ...
            desc: 'Считает каждый рубль дважды, прежде чем вложить один.'
        };
    }
    ...
    if (takenCount > 1 && !hasCapital && riskyCount >= 2) {
        return {
            ...
            desc: 'Верит, что осторожность — это медленная смерть бизнеса. Берёт по-крупному.'
        };
    }

    return {
        type: 'Бизнесмен-визионер',
        desc: 'Не складывает яйца в одну корзину. Немного риска, немного надёжности.'
    };
}
...
function initLotPage(lotId) {
    ...
    // --- ЕСЛИ ВСЕ 5 ЛОТОВ СОБРАНЫ, ПОКАЗЫВАЕМ СООБЩЕНИЕ ---
    if (isAllLotsDone()) {
        ...
        if (container) {
            container.innerHTML = `
                <div class="all-done-overlay">
                    ...
                    <div class="big-text">Собраны все 5/5 лотов!</div>
                    <div class="sub-text">Вы успешно прошли все точки. Переходите к результату!</div>
                    <a href="result.html" style="display:inline-block; margin-top:20px; padding:14px 32px; background:#DE1A1A; color:white; border-radius:60px; text-decoration:none; font-weight:600;">Перейти к результату →</a>
                </div>
            `;
        }
...
function showHint(lotId) {
    ...
    const resultBtnContainer = document.getElementById('resultButtonContainer');
    
    if (done >= 5 && isAllLotsDone()) {
        // Показываем кнопку "Узнать результат" только на lot5.html
        if (resultBtnContainer) {
            resultBtnContainer.style.display = 'block';
            
            const goToResultBtn = document.getElementById('goToResultBtn');
            if (goToResultBtn) {
                const newBtn = goToResultBtn.cloneNode(true);
                goToResultBtn.parentNode.replaceChild(newBtn, goToResultBtn);
...
function renderResult() {
    ...
    // Изображение для типажа (размеры заданы в CSS: 204×204px)
    const typeImage = TYPE_IMAGES[res.type] || '';

    document.getElementById('resultImage').innerHTML = typeImage 
        ? `<img src="${typeImage}" alt="${res.type}" class="result-image" />`
        : `<div class="result-image" style="background:#eae7e0; font-size:48px; display:flex; align-items:center; justify-content:center;">🏢</div>`;
...
function initStartPage() {
    ...
    // Если есть активная сессия и квест не завершён — предлагаем продолжить
    if (hasSession && !state.finished && !isAllLotsDone()) {
        ...
        if (nextLotId) {
            ...
            continueBtn.className = 'btn-start-extra';
    ...
    newStartBtn.className = 'btn-start-text';
    newStartBtn.textContent = startBtn.textContent || 'НАЧАТЬ';
    newStartBtn.type = 'button';
    
    newStartBtn.style.background = 'transparent';
    newStartBtn.style.border = 'none';
    newStartBtn.style.color = '#DE1A1A';
    newStartBtn.style.fontSize = '20px';
    newStartBtn.style.fontWeight = '700';
    newStartBtn.style.textDecoration = 'underline';
    newStartBtn.style.textUnderlineOffset = '4px';
    newStartBtn.style.cursor = 'pointer';
    newStartBtn.style.padding = '8px 0';
    newStartBtn.style.width = '100%';
    newStartBtn.style.display = 'block';
    newStartBtn.style.textAlign = 'center';
